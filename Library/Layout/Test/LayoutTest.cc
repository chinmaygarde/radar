...
TEST(LayoutTest, TermConstruction) {
  ...
  ASSERT_EQ(expr.terms().size(), 1u);
  ASSERT_EQ(expr.constant(), 1.0);
  ASSERT_EQ(expr2.terms().size(), 2u);
  ASSERT_EQ(expr3.terms().size(), 2u);
  ASSERT_EQ(expr4.terms().size(), 3u);
}
...
TEST(LayoutTest, DoubleConstruction) {
  ...
  ASSERT_EQ(expr2.terms().size(), 1u);
  ASSERT_EQ(expr3.terms().size(), 1u);
  ASSERT_EQ(expr4.terms().size(), 1u);
  ASSERT_EQ(term5.terms().size(), 1u);
  ASSERT_EQ(term5.terms()[0].coefficient(), -2.0);
  ASSERT_EQ(term6.terms().size(), 1u);
  ASSERT_EQ(term6.terms()[0].coefficient(), -2.0);
  ASSERT_EQ(term.terms().size(), 1u);
...
TEST(LayoutTest, ComplexOperationOverload) {
  ...
  ASSERT_EQ(expr2.constant(), 10u);
  ASSERT_EQ(expr2.terms().size(), 3u);
...
TEST(LayoutTest, ConstraintCreation) {
  ...
  ASSERT_EQ(constraint.expression().terms().size(), 1u);
  ASSERT_EQ(constraint.expression().constant(), 35u);
  ...
  ASSERT_EQ(constraint2.expression().terms().size(), 2u);
  ...
  ASSERT_EQ(constraint3.expression().terms().size(), 2u);
  ...
  ASSERT_EQ(constraint4.expression().terms().size(), 3u);
  ASSERT_EQ(constraint4.expression().constant(), 800u);
...
TEST(LayoutTest, ConstraintCreationAtPriority) {
  ...
  ASSERT_EQ(constraint2.expression().terms().size(), 1u);
  ASSERT_EQ(constraint2.expression().constant(), 35u);
...
TEST(LayoutTest, SerializeDeserializeConstraint) {
  ...
  ASSERT_EQ(readMessages.size(), 1u);
  ...
  ASSERT_EQ(decoded.expression().terms().size(), 3u);
