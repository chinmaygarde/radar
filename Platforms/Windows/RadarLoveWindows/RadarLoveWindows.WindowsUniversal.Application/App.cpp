 ...
 *  app.
 */
ref class SimpleApplicationSource sealed
...
void App::Initialize(CoreApplicationView ^ applicationView) {
  ...
  /*
   *  Logic for other event handlers could go here. Information about the
   *  Suspending and Resuming event handlers can be found here:
 ...
 *  if your IFrameworkView class is torn down while the app is in the
 *  foreground.
 */
...
void App::InitializeEGL(CoreWindow ^ window) {
  ...
  const EGLint defaultDisplayAttributes[] = {
       ...
       *  D3D11 renderer. eglInitialize will only succeed with these attributes
       *  if the hardware supports D3D11 Feature Level 10_0+.
       */
  ...
  const EGLint fl9_3DisplayAttributes[] = {
       ...
       *  Feature Level 9_3. These attributes are used if the call to
       *  eglInitialize fails with the default display attributes.
       */
  ...
  const EGLint warpDisplayAttributes[] = {
      /*
       *  These attributes can be used to request D3D11 WARP. They are used if
       *  eglInitialize fails with both the default display attributes and
       *  the 9_3 display attributes.
       */
      EGL_PLATFORM_ANGLE_TYPE_ANGLE,
      EGL_PLATFORM_ANGLE_TYPE_D3D11_ANGLE,
   ...
   * to eglGetPlatformDisplayEXT:
   * 1) The first calls uses "defaultDisplayAttributes" as a parameter. This
   *    corresponds to D3D11 Feature Level 10_0+.
   * 2) If eglInitialize fails for step 1 (e.g. because 10_0+ isn't supported by
   *    the default GPU), then we try again using "fl9_3DisplayAttributes".
   *    This corresponds to D3D11 Feature Level 9_3.
   * 3) If eglInitialize fails for step 2 (e.g. because 9_3+ isn't supported by
   *    the default GPU), then we try again using "warpDisplayAttributes".
   *    This corresponds to D3D11 Feature Level 11_0 on WARP, a D3D11 software
   *    rasterizer.
   */
  ...
  if ((eglChooseConfig(mEglDisplay, configAttributes, &config, 1,
    ...
    throw Exception::CreateException(E_FAIL,
                                     L"Failed to choose first EGLConfig");
  }
  ...
  surfaceCreationProperties->Insert(ref new String(EGLNativeWindowTypeProperty),
                                    window);
  ...
  if (mEglSurface == EGL_NO_SURFACE) {
    throw Exception::CreateException(
        E_FAIL, L"Failed to create EGL fullscreen surface");
  }

  mEglContext =
      eglCreateContext(mEglDisplay, config, EGL_NO_CONTEXT, contextAttributes);
  ...
  if (eglMakeCurrent(mEglDisplay, mEglSurface, mEglSurface, mEglContext) ==
      EGL_FALSE) {
    throw Exception::CreateException(
        E_FAIL, L"Failed to make fullscreen EGLSurface current");
  }
}
